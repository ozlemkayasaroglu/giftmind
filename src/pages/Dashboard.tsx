...
const AddPersonaModal: React.FC<{
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Log selected file and preview url
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If an avatar was selected, upload it now and log the result
      if ((created as any)?.id && avatarFile) {
        try {
          const pid = (created as any).id as string;
          const res = await railwayApi.uploadPersonaAvatar(pid, avatarFile);
          console.log("Avatar upload result:", res);
        } catch (upErr) {
