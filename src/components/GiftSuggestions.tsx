...
const GiftSuggestions: React.FC<GiftSuggestionsProps> = ({ personaId, className = '', buttonLabel = 'Get Gift Suggestions' }) => {
  const [ideas, setIdeas] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const authHeader = useMemo(() => {
    const token = localStorage.getItem('railway_token') || localStorage.getItem('authToken');
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.message || json?.error || `Failed to get suggestions (${res.status})`);
      }

      const list: string[] = json?.data ?? json?.suggestions ?? json?.recommendations ?? [];
          ...
          {ideas.map((idea, idx) => (
            ...
            >
              ...
              <div className="relative flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">{idea}</h4>
